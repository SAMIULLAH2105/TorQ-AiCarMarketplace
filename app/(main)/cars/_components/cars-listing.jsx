...
export function CarListings() {
  ...
  // Read current page and filters from URL,defaults to page 1
  const page = parseInt(searchParams.get("page") || "1");

  // Use the useFetch hook
  const { loading, fn: fetchCars, data: result, error } = useFetch(getCars);

  // Fetch cars based on current filters and page
  useEffect(() => {
    fetchCars({
      search,
      make,
  ...
  // Update URL when page currentPage changes
  useEffect(() => {
    if (currentPage !== page) {
  ...
  const handlePageChange = (pageNum) => {
    setCurrentPage(pageNum);// triggers useEffect above
  };
