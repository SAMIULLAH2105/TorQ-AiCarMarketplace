  ...
  try {
    ...
    if (!dealership) {
      dealership = await db.dealershipInfo.create({
        ...
        /*
        Tells Prisma to also return the related workingHours records after creating the dealership. 
        orderBy ensures the workingHours come back sorted by dayOfWeek ascending (e.g., MONDAY → SUNDAY).
        */
        include: {
          workingHours: {
            orderBy: {
...
export async function saveWorkingHours(workingHours) {
  try {
    ...
    // revalidatePath() is used to refresh a specific page on the server so that it shows the latest data.
